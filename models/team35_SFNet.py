...
class FKM(nn.Module):
    def __init__(self, in_channels, out_channels, groups=4):
        ...
        self.conv_low = nn.Conv2d(in_channels * 2, in_channels * 2, kernel_size=1, stride=1, padding=0, groups=in_channels * 2)
        self.conv_mid = nn.Conv2d(in_channels * 2, in_channels * 2, kernel_size=1, stride=1, padding=0, groups=in_channels * 2)
        self.conv_high = nn.Conv2d(in_channels * 2, in_channels * 2, kernel_size=1, stride=1, padding=0, groups=in_channels * 2)
        self.alpha = nn.Parameter(torch.zeros(in_channels * 2, 1, 1))

    def _generate_masks(self, h, w):
        center_h, center_w = h // 2, w // 2
        low_radius = min(h, w) // 8
        mid_radius = min(h, w) // 4
        y, x = torch.meshgrid(torch.arange(h), torch.arange(w // 2 + 1))
        dist = ((y - center_h) ** 2 + (x - center_w) ** 2).sqrt()
        mask_low = (dist <= low_radius).unsqueeze(0).unsqueeze(0)  # Low-frequency mask
        mask_mid = ((dist > low_radius) & (dist <= mid_radius)).unsqueeze(0).unsqueeze(0)  # Mid-frequency mask
        mask_high = (dist > mid_radius).unsqueeze(0).unsqueeze(0)  # High-frequency mask
        return mask_low, mask_mid, mask_high

    def forward(self, x):
        batch, c, h, w = x.size()
        mask_low, mask_mid, mask_high = self._generate_masks(h, w)
        ...
        r1 = ffted
        mask_low = mask_low.to(x.device).repeat(batch, 2 * c, 1, 1)
        mask_mid = mask_mid.to(x.device).repeat(batch, 2 * c, 1, 1)
        mask_high = mask_high.to(x.device).repeat(batch, 2 * c, 1, 1)
        low_band = self.conv_low(ffted * mask_low)
        mid_band = self.conv_mid(ffted * mask_mid)
        high_band = self.conv_high(ffted * mask_high)
        ffted = (low_band + mid_band + high_band) * self.alpha + r1

        ffted = self.bn(ffted)
        dy_weight = self.weight(ffted)
        ffted = self.fpe(ffted)
...
class SFNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.freblock_1 = FKM(feature_channels, feature_channels)
        self.block_2 = SPAB(feature_channels, bias=bias)
        self.freblock_2 = FKM(feature_channels, feature_channels)
        self.block_3 = SPAB(feature_channels, bias=bias)
        self.freblock_3 = FKM(feature_channels, feature_channels)
        self.block_4 = SPAB(feature_channels, bias=bias)
        self.freblock_4 = FKM(feature_channels, feature_channels)
        self.block_5 = SPAB(feature_channels, bias=bias)
        self.freblock_5 = FKM(feature_channels, feature_channels)
        self.block_6 = SPAB(feature_channels, bias=bias)
        self.freblock_6 = FKM(feature_channels, feature_channels)
    ...
    def forward(self, x):
        ...
        fre_b1 = self.freblock_1(out_feature)
        out_b1 = out_b1 + fre_b1

        out_b2, _, att2 = self.block_2(out_b1)
        fre_b2 = self.freblock_2(out_b1)
        out_b2 = out_b2 + fre_b2

        out_b3, _, att3 = self.block_3(out_b2)
        fre_b3 = self.freblock_3(out_b2)
        out_b3 = out_b3 + fre_b3

        out_b4, _, att4 = self.block_4(out_b3)
        fre_b4 = self.freblock_4(out_b3)
        out_b4 = out_b4 + fre_b4

        out_b5, _, att5 = self.block_5(out_b4)
        fre_b5 = self.freblock_5(out_b4)
        out_b5 = out_b5 + fre_b5

        out_b6, out_b5_2, att6 = self.block_6(out_b5)
        fre_b6 = self.freblock_6(out_b5)
        out_b6 = out_b6 + fre_b6
